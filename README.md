...
| 设定 | span-F1 | P | R | 本字 Acc（oracle） |
|------|--------:|--:|--:|-------------------:|
| 词典匹配基线 | 0.134 | 0.072 | 1.000 | 0.650 |
| 金标全参 v1 | 0.493 | 0.602 | 0.418 | 0.635 |
| **主模型：金标全参 v2** | **0.546** | 0.580 | 0.516 | 0.653 |
| 金标 LoRA | 0.300 | 0.629 | 0.197 | 0.658 |
| 全参 + 银标负例预热 | 0.468 | 0.636 | 0.370 | 0.631 |

- 相对词典基线，主模型 span-F1 约 **0.13 → 0.55**（约 4 倍）  
- 高频 Top-50 通假字对上，主模型 F1 ≈ **0.87**  
- 银标伪标正例抽检可用率约 **2%**，故**不参与主模型训练**；负例仅用于轻量 warmup 消融  

指标 JSON 见 `checkpoints/*/metrics_test.json`（训练得到的 `.pt` 权重默认不提交）。
